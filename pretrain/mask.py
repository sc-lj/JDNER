...
class Mask(object):
    def __init__(self, same_py_confusion, simi_py_confusion, sk_confusion, tokenid2pyid, tokenid2skid):
        ...
        self.config = {'same_py': 0.3, 'simi_py': 0.3, 'stroke': 0.15,
                       'random': 0.1, 'keep': 0.15, 'global_rate': 0.15}
        self.same_py_thr = self.config['same_py']
        self.simi_py_thr = self.config['same_py'] + self.config['simi_py']
        self.stroke_thr = self.config['same_py'] + \
            self.config['simi_py'] + self.config['stroke']
        self.random_thr = self.config['same_py'] + self.config['simi_py'] + \
    ...
    def mask_process(self, input_sample):
        valid_ids = [idx for (idx, v) in enumerate(
            input_sample) if v not in self.invalid_ids]
        ...
        for pos in valid_ids[:n_masked]:
            ...
            if method == 'pinyin':
                ...
                new_c = self.sk_confusion.get_confusion_item_by_ids(
                    input_sample[pos])
                if new_c is not None:
                    masked_sample[pos] = new_c
                    masked_flgs[pos] = 1
            elif method == 'random':
                new_c = self.all_token_ids[random.randint(
                    0, self.n_all_token_ids)]
                if new_c is not None:
                    masked_sample[pos] = new_c
                    masked_flgs[pos] = 1
            elif method == 'keep':
