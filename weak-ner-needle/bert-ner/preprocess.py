...
class DataProcessor(object):
    ...
    def __init__(self, data_dir, features=[]):
        ...
        self._label_types = self._read_label(
        ...
        self._invsered_label_map = {i: label for i,
                                    label in enumerate(self._label_types)}
                ...
                for line in lines:
                    ...
                    for feature_col in self.features:
                        ...
                        if cols[feature_col] not in self.feature_value_idx_map[feature_col]:
                            self.feature_value_idx_map[feature_col][cols[feature_col]] = max(
                                len(self.feature_value_idx_map[feature_col]) - 1, 0)
        for feature in self.features:
            if feature != 1:
                self.feature_dim_dict[feature] = len(
                    self.feature_value_idx_map[feature]) - 1
            else:
                self.feature_dim_dict[feature] = len(
                    self.feature_value_idx_map[feature])
            ...
            for entry in tqdm(entries):
                ...
                for line in lines:
                    ...
                    for idx in self.features:
                        if idx > len(cols):
                            raise RuntimeError(
                                "feature idx %d greater than number of columns in line %s " % (idx, line))
                        if cols[idx] in self.feature_value_idx_map[idx]:
                            feature_values[idx].append(
                                self.feature_value_idx_map[idx][cols[idx]])
                        else:
                            feature_values[idx].append(
                                self.feature_value_idx_map[idx]["UNKNOWN"])
