...
class NERTrainer(Trainer):
    ...
    def prediction_step(self, *args, **kwargs):
        ...
        labels = torch.cat([labels, torch.zeros(
            (b, maxl - l)).fill_(pad_token_label_id).to(labels)], dim=1)
        logits = torch.cat([logits, torch.zeros(
            (b, maxl - l, c)).fill_(-20000.0).to(logits)], dim=1)
...
def main(args):
    ...
    logging.basicConfig(format=LOG_FORMAT, level=logging.INFO,
                        filename=logger_name, filemode='w')
    ...
    train_examples = nerProcessor.get_examples(
        os.path.join(data_args.data_dir, data_args.train_split))
    if data_args.use_da:
        train_examples.extend(nerProcessor.get_examples(
            os.path.join(data_args.data_dir, "aug_train.txt")))
    ...
    if data_args.weak_file is not None:
        ...
        if data_args.weak_wei_file is not None:
            ...
            train_examples_wei.extend(
                list(np.load(data_args.weak_wei_file, allow_pickle=True)))
            assert len(train_examples) + \
                len(weak_examples) == len(train_examples_wei)
        train_examples.extend(weak_examples)
    if train_examples_wei is None:
        train_examples_wei = [1] * len(train_examples)
    train_examples_wei = [min(data_args.max_weight, w)
                          for w in train_examples_wei]
    dev_examples = nerProcessor.get_examples(
        os.path.join(data_args.data_dir, data_args.dev_split))
    test_examples = nerProcessor.get_examples(
        os.path.join(data_args.data_dir, data_args.test_split))
    ...
    if not hasattr(config, 'features_dict'):
        ...
        if model_args.feature_names is not None:
            ...
            feature_list = [int(featureName2idx[feature])
    ...
    def align_predictions(predictions: np.ndarray, label_ids: np.ndarray) -> Tuple[List[int], List[int]]:
        ...
        def f(k, label_ids, preds):
            ...
            return r
        preds_list = ray.get([f.remote(i, label_ids, preds)
                             for i in range(100)])
        ...
        def f(k, label_ids, preds):
            ...
            for i in range(chunksize * k, min(batch_size, chunksize * (k + 1))):
                ...
                for j in range(seq_len):
                    if label_ids[i, j] != nn.CrossEntropyLoss().ignore_index:
                        out_label_list_sub.append(
                            inversed_label_map[label_ids[i][j]])
                r.append(out_label_list_sub)
            return r
        out_label_list = ray.get(
            [f.remote(i, label_ids, preds) for i in range(100)])
    ...
    def compute_metrics(p: EvalPrediction, full: Optional[bool] = False, skip: Optional[bool] = False) -> Dict:
        ...
        metrics = compute_accuracy_labels(
            out_label_list, preds_list, bio_format=True, ignore_labels=set(["category"]))
        result = {
    ...
    if data_args.do_profile:
        ...
        if data_args.profile_file == "dev":
            ...
            profile_examples = nerProcessor.get_examples(
                data_args.profile_file)
        ...
        for exid in tqdm.tqdm(range(0, len(profile_examples), chunk_size)):
            chunk_examples = profile_examples[exid:min(
                len(profile_examples), exid + chunk_size)]
            profile_dataset = NerDataset(
                chunk_examples, tokenizer, label_map, data_args.max_seq_length)
            ...
            if trainer.is_world_process_zero():
                ...
                decrepted_samples = len(
                    [x for x in chunk_profile_data if len(x[-1]) != len(x[-2])])
                if decrepted_samples > 0:
                    print(
                        f"Warning: {decrepted_samples} instances are cutted off but still save all labels, words")
                profile_data.extend(chunk_profile_data)
        if trainer.is_world_process_zero():
            assert len(profile_data) == len(profile_examples)
            pickle.dump(profile_data, open(os.path.join(
                training_args.output_dir, profile_tag + 'profile_data.pickle'), 'wb'))
            print("Query Level Accuracy:  {:.4f}".format(
                sum([x[3] for x in profile_data]) / len(profile_data)))
    ...
    if data_args.pred_file is not None:
        ...
        for exid in tqdm.tqdm(range(0, len(pred_examples), chunk_size)):
            ...
            if trainer.is_world_process_zero():
                chunk_preds_list, chunk_out_label_list = align_predictions(
                ...
                for ps, ls, es in tqdm.tqdm(zip(preds_list, out_label_list, pred_examples)):
                    ...
                    for p, l, e in zip(ps, ls, es.words):
                        if p != l and l != 'O':
                            ...
                            print(
                                'Not Match Weak {} -> Pred {} \t\t {} \t\t {}'.format(l, p, e, " ".join(es.words)))
                        p = save_rule(data_args.save_pred_rule, p, l)
                        if p.startswith('I-'):
                            if prevp != p and prevp != p.replace('I-', 'B-'):
                                print(
                                    'Error {} -> {} \t\t {} -> {} \t\t {}'.format(prevp, p, preve, e, " ".join(es.words)))
                                total_error_nums += 1
                        prevp = p
                        preve = e
                        fout.write("{}\t{}\n".format(e, p))
