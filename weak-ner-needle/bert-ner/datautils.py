...
def example2feature(example, tokenizer, label_map, max_seq_length, weight=1):
    ...
    if len(tokens) > max_seq_length - 1:
        print('Example No.{} is too long, length is {}, truncated to {}!'.format(
            example.guid, len(tokens), max_seq_length))
...
class NerDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        feat = example2feature(
            self.examples[idx], self.tokenizer, self.label_map, self.max_seq_length, self.weights[idx])
        return feat
    ...
    def pad(cls, batch):
        ...
        attention_mask_list = torch.LongTensor(
            [sample.attention_mask for sample in batch])
        token_type_ids_list = torch.LongTensor(
            [sample.token_type_ids for sample in batch])
        label_ids_list = torch.LongTensor(
            [sample.label_ids for sample in batch])
        features_list = torch.LongTensor([sample.features for sample in batch])
        predict_mask_list = torch.LongTensor(
            [sample.predict_mask for sample in batch])
    ...
    def dynamic_collator(cls, pad_token_id):
        ...
        class DyCollator:
            ...
            def __call__(self, batch: List[InputFeatures]) -> Dict[str, torch.Tensor]:
                ...
                lens = (input_ids[valid_samples] !=
                        self.pad_token_id).long().sum(-1)
