...
def compute_accuracy_qformat(gold_file, ref_file, delimiter="\t", header=False,
    ...
    with open(gold_file, encoding="utf-8") as gold_in, open(ref_file, encoding="utf-8") as ref_in:
        ...
        for ind, (gold_row, ref_row) in enumerate(zip_longest(gold_reader, ref_reader)):
            if not gold_row or not ref_row:
                raise RuntimeError(
                    "gold file or reference file line is null at line %d !" % (ind + 1))
            if gold_row[keywords_col] != ref_row[keywords_col]:
                raise RuntimeError(
                    "keywords of gold file and keywords of reference file are different at line %d " % (ind + 1))
            ...
            if len(tokens) != len(current_gold_labels) or len(current_gold_labels) != len(current_ref_labels):
                raise RuntimeError(
                    "gold file token and reference file token, label size are different at line %d" % (ind + 1))
    ...
    metrics = compute_accuracy_labels(
        gold_labels, ref_labels, keywords, bio_format, ignore_labels)
    return metrics
    ...
    with open(gold_file, encoding="utf-8") as gold_in, open(ref_file, encoding="utf-8") as ref_in:
        ...
        for line_gold, line_ref in zip_longest(gold_in, ref_in):
            if bool(line_gold.strip()) != bool(line_ref.strip()):
                raise RuntimeError(
                    "gold file and reference file line mismatch %s, %s" % (line_gold, line_ref))
            if line_gold.strip():
                ...
                if cols_gold[token_col] != cols_ref[token_col]:
                    raise RuntimeError(
                        "gold file and reference file token mismatch %s, %s " % (line_gold, line_ref))
...
def compute_accuracy_labels(gold_labels, ref_labels, keywords=None, bio_format=True, ignore_labels=set([]), full_stats=False):
    ...
    if len(gold_labels) != len(ref_labels) or (keywords and len(gold_labels) != len(keywords)):
        raise RuntimeError(
            "size of gold labels, ref labels and keywords is not the same!")
    ...
    for i in range(len(gold_labels)):
        ...
        for j in range(len(current_gold_labels)):
            ...
            token_label_match = int(
                current_gold_label_key == current_ref_label_key)
            ...
            if current_gold_label_key not in ignore_labels:
                ...
                fn_label_token_cnt[current_gold_label_key] += (
                    1 - token_label_match)

            if current_ref_label_key not in ignore_labels:
                label_set.add(current_ref_label_key)
                fp_label_token_cnt[current_ref_label_key] += (
                    1 - token_label_match)
                if current_ref_label_key != OUTSIDE:
                    annotated_tokens += 1

            if current_gold_label_key != current_ref_label_key:
                label_confusion[(current_gold_label_key,
                                 current_ref_label_key)] += 1
                if log_error_keywords:
                    label_confusion_query[(current_gold_label_key, current_ref_label_key)].add(
                        (current_keywords[j], concat_keywords))
    ...
    for label in label_set:
        token_precision[label] = div(
            tp_label_token_cnt[label], (tp_label_token_cnt[label] + fp_label_token_cnt[label]))
        token_recall[label] = div(
            tp_label_token_cnt[label], (tp_label_token_cnt[label] + fn_label_token_cnt[label]))
        token_f1[label] = (token_precision[label] + token_recall[label]) / 2
        span_precision[label] = div(
            tp_label_span_cnt[label], (tp_label_span_cnt[label] + fp_label_span_cnt[label]))
        span_recall[label] = div(
            tp_label_span_cnt[label], (tp_label_span_cnt[label] + fn_label_span_cnt[label]))
    ...
    mean_token_precision = div(
        total_tp_token_prediction, total_tp_token_prediction + total_fp_token_prediction)
    mean_token_recall = div(total_tp_token_prediction,
                            total_tp_token_prediction + total_fn_token_prediction)
    mean_span_precision = div(
        total_tp_span_prediction, total_tp_span_prediction + total_fp_span_prediction)
    mean_span_recall = div(total_tp_span_prediction,
                           total_tp_span_prediction + total_fn_span_prediction)
        ...
        if len(spans) == 0:
            ...
            if not bio_format:
                ...
                if labels[j].startswith(BEGIN):
                    ...
                    raise RuntimeError(
                        "wrong bioformat label %s !" % (labels[j]))
