...
# a list of tags, and a viterbi score
...
def allowed_transitions(constraint_type: str, labels: Dict[int, str]) -> List[Tuple[int, int]]:
    ...
    labels_with_boundaries = list(
        labels.items()) + [(start_tag, "START"), (end_tag, "END")]
    ...
    if constraint_type == "BIOUL":
        ...
        return any(
            [
                ...
                from_tag in ("B", "I") and to_tag in (
                    "I", "L") and from_entity == to_entity,
            ]
        )
        ...
        return any(
            [
                ...
                # x is the same tag.
                to_tag == "M" and from_tag in (
                    "B", "M") and from_entity == to_entity,
                # Can only transition to E-x from B-x or M-x, where
                # x is the same tag.
                to_tag == "E" and from_tag in (
                    "B", "M") and from_entity == to_entity,
            ]
        )
    else:
...
class ConditionalRandomField(torch.nn.Module):
    ...
    def __init__(
        ...
        if constraints is None:
            ...
            constraint_mask = torch.Tensor(
                num_tags + 2, num_tags + 2).fill_(1.0)
        else:
            constraint_mask = torch.Tensor(
                num_tags + 2, num_tags + 2).fill_(0.0)
    ...
    def _joint_likelihood(
        ...
        for i in range(sequence_length - 1):
            ...
            transition_score = self.transitions[current_tag.view(
                -1), next_tag.view(-1)]
        ...
        last_tags = tags.gather(
            0, last_tag_index.view(1, batch_size)).squeeze(0)
