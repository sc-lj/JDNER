...
class RelativePositionsEncoding(nn.Module):
    def __init__(self, length, depth, max_relative_position=127):
        ...
        div_term = torch.exp(torch.arange(0, depth, 2).float()
                             * (-math.log(10000.0) / depth))
        embeddings_table[:, 0::2] = torch.sin(position * div_term)
        embeddings_table[:, 1::2] = torch.cos(position * div_term)
        embeddings_table = embeddings_table.unsqueeze(
...
class NeZhaEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList([NeZhaLayer(config)
...
class NeZhaPreTrainedModel(PreTrainedModel):
    ...
    def save_pretrained(
        ...
        state_dict = {k: v for k, v in state_dict.items()
                      if 'relative_positions' not in k}
...
class NeZhaForMaskedLM(NeZhaPreTrainedModel):
    ...
    Returns:
        ...
        # (ltr_lm_loss), (masked_lm_loss), prediction_scores, (hidden_states), (attentions)
        return outputs
