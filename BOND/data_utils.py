    ...
    with open(file_path, 'r') as f:
        ...
        for item in data:
            ...
            examples.append(InputExample(guid="%s-%d".format(mode, guid_index),
                            words=words, labels=labels, hp_labels=hp_labels))
            guid_index += 1
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for word, label, hp_label in zip(example.words, example.labels, example.hp_labels):
            ...
            label_ids.extend([label] + [pad_token_label_id]
                             * (len(word_tokens) - 1))
            hp_label_ids.extend([hp_label if hp_label is not None else pad_token_label_id] + [
        ...
        if pad_on_left:
            ...
            hp_label_ids = ([pad_token_label_id] *
                            padding_length) + hp_label_ids
            full_label_ids = ([pad_token_label_id] *
                              padding_length) + full_label_ids
        else:
        ...
        if ex_index < show_exnum:
            ...
            logger.info("hp_label_ids: %s", " ".join(
                [str(x) for x in hp_label_ids]))
            logger.info("full_label_ids: %s", " ".join(
                [str(x) for x in full_label_ids]))

        features.append(
            InputFeatures(input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids,
                          label_ids=label_ids, full_label_ids=full_label_ids, hp_label_ids=hp_label_ids)
        )
    logger.info("Extra long example %d of %d",
                extra_long_samples, len(examples))
    return features


def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode, remove_labels=False):
    evaluate = False if mode != "dev" else True
    ...
    cached_features_file = os.path.join(
        args.data_dir,
        "cached_{}_{}_{}".format(
            mode, list(filter(None, args.model_name_or_path.split("/"))
                       ).pop(), str(args.max_seq_length)
        ),
    )
    ...
    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                            all_label_ids, all_full_label_ids, all_hp_label_ids, all_ids)
    return dataset
