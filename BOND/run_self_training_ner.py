...
def train(args, train_dataset, model_class, config, tokenizer, labels, pad_token_label_id):
    ...
    model, optimizer, scheduler = initialize(
        args, model_class, config, t_total, 0)
    # Train!
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if global_step >= args.self_training_begin_step:
                ...
                if args.self_training_label_mode == "hard":
                    ...
                    pred_labels, label_mask = multi_source_label_refine(
                        args, batch[5], batch[3], pred_labels, pad_token_label_id, pred_logits=outputs[0])
                elif args.self_training_label_mode == "soft":
                    pred_labels = soft_frequency(logits=outputs[0], power=2)
                    pred_labels, label_mask = multi_source_label_refine(
                        args, batch[5], batch[3], pred_labels, pad_token_label_id)

                inputs = {"input_ids": batch[0], "attention_mask": batch[1],
                          "labels": pred_labels, "label_mask": label_mask}
            else:
                inputs = {
            ...
            if args.mt and global_step % args.mt_updatefreq == 0:
                ...
                ), model.named_parameters(), args.mt_avg, alpha, update_step)
            ...
            if args.mt and update_step > 0:
                ...
                if args.mt_loss_type == "embeds":
                    mt_loss = get_mt_loss(
                        final_embeds, teacher_final_embeds.detach(), args.mt_class, _lambda)
                else:
                    mt_loss = get_mt_loss(
            ...
            if args.vat:
                ...
                if not word_embed:
                    ...
                    vat_embeds = (embeds.data.detach(
                    ) + embeds.data.new(embeds.size()).normal_(0, 1)*1e-5).detach()
                    vat_embeds.requires_grad_()

                    vat_inputs = {"inputs_embeds": vat_embeds,
                    ...
                    vat_embeds.grad = opt_grad(
                        vat_loss, vat_embeds, optimizer)[0]
                    ...
                    if (torch.isnan(norm) or torch.isinf(norm)):
                        ...
                        if args.vat_loss_type == "embeds":
                            ...
                            vat_loss = get_mt_loss(vat_logits, logits.detach(), args.mt_class, args.vat_lambda) \
                                + get_mt_loss(logits, vat_logits.detach(),
                                              args.mt_class, args.vat_lambda)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.evaluate_during_training:
                        ...
                        results, _, best_dev, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id, best_dev, mode="dev",
                                                           prefix='dev [Step {}/{} | Epoch {}/{}]'.format(global_step, t_total, epoch, args.num_train_epochs), verbose=False)
                        for key, value in results.items():
                            tb_writer.add_scalar(
                                "eval_{}".format(key), value, global_step)

                        results, _, best_test, is_updated = evaluate(args, model, tokenizer, labels, pad_token_label_id, best_test, mode="test",
...
def parse_arguments():
    ...
    parser.add_argument('--mt_updatefreq', type=int, default=1,
    ...
    parser.add_argument('--mt_lambda', type=float, default=1,
                        help="trade off parameter of the consistent loss.")
    parser.add_argument('--mt_rampup', type=int,
                        default=300, help="rampup iteration.")
    ...
    parser.add_argument('--mt_alpha2', default=0.995, type=float,
                        help="moving average parameter of mean teacher (for the exponential moving average).")
    parser.add_argument('--mt_beta', default=10, type=float,
    ...
    parser.add_argument('--self_training_period', type=int,
                        default=878, help='the self-training period.')
    parser.add_argument('--self_training_hp_label', type=float,
                        default=0, help='use high precision label.')
    parser.add_argument('--self_training_ensemble_label',
                        type=int, default=0, help='use ensemble label.')

    # Use data from weak.json
    parser.add_argument('--load_weak', action="store_true",
                        help='Load data from weak.json.')
...
def main():
    ...
    if args.do_train:
        ...
        if args.load_weak:
            weak_dataset = load_and_cache_examples(
                args, tokenizer, labels, pad_token_label_id, mode="weak", remove_labels=args.remove_labels_from_weak)
            train_dataset = torch.utils.data.ConcatDataset(
                [train_dataset]*args.rep_train_against_weak + [weak_dataset, ])

        model, global_step, tr_loss, best_dev, best_test = train(
            args, train_dataset, model_class, config, tokenizer, labels, pad_token_label_id)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, _, best_dev, _ = evaluate(
                args, model, tokenizer, labels, pad_token_label_id, best=best_dev, mode="dev", prefix=global_step)
            if global_step:
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        if not best_test:
            best_test = [0, 0, 0]
        result, predictions, _, _ = evaluate(
            args, model, tokenizer, labels, pad_token_label_id, best=best_test, mode="test")
        # Save results
        ...
        with open(output_test_predictions_file, "w") as writer:
            with open(os.path.join(args.data_dir, "test.json"), "r") as f:
                ...
                for item in data:
                    output_line = str(
                        item["str_words"]) + " " + predictions[example_id].pop(0) + "\n"
